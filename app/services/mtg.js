...
const isUgly = card =>
  ...
  [
    ...
    'premium_deck',
    'from_the_vault'
  ].includes(card.set.type);
const hasErrata = card =>
  card.originalText !== card.text;
export const preferredPrinting = (
  a,
  b
) =>
  sortInOrder(
    ...
    sortByIndex(FRAME_VERSION_ORDER)(
      a.frameVersion,
      b.frameVersion
    ),
    sortByIndex(SET_TYPE_ORDER)(
      a.set.type,
      b.set.type
    ),
    sortByValue(
...
const firstType = card =>
  card.types.find(x =>
    TYPE_ORDER.includes(x)
  );
export const decklistSort = (a, b) => {
  const [aPrinting] = a.printings;
  const [bPrinting] = b.printings;
  return sortInOrder(
    sortByIndex(TYPE_ORDER)(
      firstType(aPrinting),
      firstType(bPrinting)
    ),
    sortByValue(
      parseInt(
        aPrinting.convertedManaCost
      ),
      parseInt(
        bPrinting.convertedManaCost
      )
    ),
    sortByValue(
      aPrinting.name,
